...
theme.init = function() {
  ...
  if(Shopify.designMode){    
    ...
    if($action_list.find('#ktImport').length == 0 ){
      ...
      // $action_list.prepend('<li><a href="#" id="ktCloneCollectionLayout" class="theme-editor-action-list__item" style="white-space: nowrap;">Clone Shop layout</a></li>');
    }
    ...
    // var $cloneBtn = window.top.document.querySelector('#ktCloneCollectionLayout');
    // $cloneBtn.addEventListener('click',function(){
    //   if(window.parent.document.getElementById("t4_clone_submit") == null){
    //     if (window.parent && window.parent.Shopify.Flash && "function" === typeof window.parent.Shopify.Flash.notice) {
    //       window.parent.Shopify.Flash.notice("Loading ...")
    //     }
    //     $.ajax({
    //       type: "GET",
    //       url: '/cart?view=kt-clone-collection&design_theme_id='+Shopify.theme.id,
    //       dataType: 'html',
    //       success: function(data) {
    //         $top.find('.te-preview').append(data).addClass('open-import');
    //       }
